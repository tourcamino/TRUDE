...
async function testExtremeNumbers() {
  ...
  const extremeTests = [
    ...
    { name: 'Negative amount', amount: '-1000000' },
    { name: 'Very large amount', amount: '999999999999999999999999999999' },
    { name: 'Decimal precision', amount: '0.000000000000000001' },
    { name: 'Scientific notation', amount: '1e18' },
    { name: 'String instead of number', amount: 'not-a-number' },
...
async function testExtremeTimestamps() {
  ...
  const timestampTests = [
    { name: 'Unix epoch (1970)', deadline: 0 },
    { name: 'Far future', deadline: 4102444800 }, // 2100
    { name: 'Far past', deadline: -86400 }, // Yesterday in 1970
    { name: 'Current time', deadline: Math.floor(Date.now() / 1000) },
    { name: 'String timestamp', deadline: 'not-a-timestamp' },
    { name: 'Float timestamp', deadline: 1234567890.123 },
    { name: 'Negative large', deadline: -9999999999 },
...
async function testMaliciousStrings() {
  ...
  const maliciousTests = [
    { name: 'SQL Injection', symbol: "'; DROP TABLE users; --" },
    { name: 'XSS Attempt', symbol: '<script>alert("XSS")</script>' },
    { name: 'JSON Injection', symbol: '{"malicious": true, "data": null}' },
    { name: 'Path Traversal', symbol: '../../../etc/passwd' },
    { name: 'Command Injection', symbol: '$(rm -rf /)' },
    { name: 'Very long string', symbol: 'A'.repeat(10000) },
    { name: 'Control characters', symbol: '\x00\x01\x02\x03\x04\x05' },
    { name: 'Mixed encoding', symbol: 'Hello\u0000World\uFFFFTest' }
  ];
...
async function testExtremeConcurrency() {
  ...
  for (const level of concurrencyLevels) {
    ...
    console.log(`${colors.gray}Time: ${duration}ms${colors.reset}`);
